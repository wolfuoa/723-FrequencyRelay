...
#define FREQPLT_GRID_SIZE_X                                                    \
  80 // pixel separation in the x axis between two data points
...
int VGA_Init() {
  ...
  if (xTaskCreate(PRVGADraw_Task, "DrawTsk", configMINIMAL_STACK_SIZE, NULL,
                  PRVGADraw_Task_P, &PRVGADraw) != pdPASS) {
    return 1;
  };
  return 0;
}
...
void PRVGADraw_Task(void *pvParameters) {
  ...
  alt_up_pixel_buffer_dma_draw_hline(
      pixel_buf, 100, 500, 200, ((0x3ff << 20) + (0x3ff << 10) + (0x3ff)), 0);
  alt_up_pixel_buffer_dma_draw_hline(
      pixel_buf, 100, 500, 300, ((0x3ff << 20) + (0x3ff << 10) + (0x3ff)), 0);
  ...
  while (1) {
    ...
    if (xQueueReceive(Q_PerformanceMeasure, &performance_mesaure_to_print,
      ...
      sprintf(ThreshStr, "%d ms",
              (running_performance_total / numb_running_index));
    ...
    if (xQueueReceive(Q_SystemStatus, &currentVgaSystemStatus, portMAX_DELAY) ==
        pdTRUE) {
    ...
    for (j = 0; j < 5; ++j) { // i here points to the oldest data, j loops
                              // through all the data to be drawn on VGA
      if (((int)(freq[(i + j) % 100]) > MIN_FREQ) &&
          ((int)(freq[(i + j + 1) % 100]) > MIN_FREQ)) {
        // Calculate coordinates of the two data points to draw a line in
        // between Frequency plot
        ...
        line_roc.y1 =
            (int)(ROCPLT_ORI_Y - ROCPLT_ROC_RES * dfreq[(i + j) % 100]);

        line_roc.x2 = ROCPLT_ORI_X + ROCPLT_GRID_SIZE_X * (j + 1);
        line_roc.y2 =
            (int)(ROCPLT_ORI_Y - ROCPLT_ROC_RES * dfreq[(i + j + 1) % 100]);
