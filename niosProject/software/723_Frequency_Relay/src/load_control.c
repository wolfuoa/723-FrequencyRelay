  ...
  while (1) {
    ...
    if (xQueueReceive(Load_Control_Q, &action, portMAX_DELAY) == pdTRUE) {
      if (xSemaphoreTake(SystemStatusMutex, (TickType_t)10) == pdTRUE) {
        ...
        xQueueReceive(Switch_Polling_Q, &tempSwitchStatus, (TickType_t)0);

        // Another try:
        // uint8_t recv;
        // if (xQueueReceive(Switch_Polling_Q, &recv, (TickType_t)0) == pdTRUE)
        // {
        //   tempSwitchStatus = recv;
        // }
        ...
        // localSwitchStatus, Load_Control_loads, (localSwitchStatus &
        // Load_Control_loads));
        ...
        if (spammingTimestampFlag) {
          if (xSemaphoreTake(Peak_Detector_performanceTimerMutex_X,
            ...
            if (g_peakDetectorPerformanceTimestamp != 0) {
              int entry =
                  (alt_timestamp() - g_peakDetectorPerformanceTimestamp) / 1000;
