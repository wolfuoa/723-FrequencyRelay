...
int Peak_Detector_init() {
  ...
  if (xTaskCreate(Peak_Detector_handlerTask, "Peak_Detector_T",
                  configMINIMAL_STACK_SIZE, NULL,
                  PEAK_DETECTOR_HANDLER_PRIORITY, NULL) != pdPASS) {
    return 1;
  }
  return 0;
}
...
static void Peak_Detector_handlerTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(Peak_Detector_Q, &temp, portMAX_DELAY) == pdTRUE) {
      ...
      frequencyDeltaBuf[frequencyBufIndex] =
          frequencyReading - previousFrequency;
      ...
      rateOfChangeReading = 1000 * (rateOfChangeReading / 10) /
                            (double)(timestampForROC - previousTimestamp);
      // rateOfChangeReading = 1000 * ((frequencyReading - previousFrequency) /
      // (double)((timestampForROC - previousTimestamp)));

      // printf("Difference - %d, ROC: %f, index: %d\n", timestampForROC -
      // previousTimestamp, rateOfChangeReading, frequencyBufIndex);
      previousTimestamp = timestampForROC;

      // Replace previousFrequency
      previousFrequency = frequencyReading;

      VGA_Stats currentVGAStats = {frequencyReading, rateOfChangeReading};

      // VGA Queue
      xQueueSendToBack(Q_VGA_Stats, &currentVGAStats, pdFALSE);

      // Dont change the thresholds while we are checking the thresholds
      if (xSemaphoreTake(Peak_Detector_thresholdMutex_X, (TickType_t)10) ==
          pdTRUE) {
        //                printf("FreqL %f, FreqH %f, RocL %f, RocH %f\n",
        //                g_peakDetectorLowerFrequencyThreshold,
        ...
        thresholdEval =
            ((frequencyReading > g_peakDetectorLowerFrequencyThreshold) &&
             (frequencyReading < g_peakDetectorHigherFrequencyThreshold) &&
             (rateOfChangeReading >= g_peakDetectorLowerROCThreshold) &&
             (rateOfChangeReading < g_peakDetectorHigherROCThreshold));
        // printf("stable? %d\n", thresholdEval);
      ...
      if (xSemaphoreTake(repeatActionMutex_X, (TickType_t)10) == pdTRUE) {
        ...
        if (xSemaphoreTake(Peak_Detector_debounceMutex_X, (TickType_t)10) ==
            pdTRUE) {
          // If system status is stable and goes outside threshold
          if ((systemStability == SYSTEM_FREQUENCY_STATE_STABLE) &&
              (thresholdEval == SYSTEM_FREQUENCY_STATE_UNSTABLE) &&
              g_peakDetectorDebounceFlag == 0) {
            g_peakDetectorDebounceFlag = 1;

            if (xSemaphoreTake(Peak_Detector_performanceTimerMutex_X,
          ...
          // If system status is unstable and goes inside threshold
            ...
            if (xTimerStart(repeatActionTimer, 0) != pdPASS) {
              printf("Cannot reset timer\n\r");
                     ...
                     (thresholdEval == SYSTEM_FREQUENCY_STATE_STABLE)) {
            xQueueSendToBack(Load_Control_Q, &thresholdEval, pdFALSE);
...
static void Peak_Detector_initDataStructs() {
  ...
  // Important: 500 is the amount of ticks. To convert to ms, divide by
  // portTICK_PERIOD_MS
  repeatActionTimer =
      xTimerCreate("Repeat_Action_Timer", 500 / portTICK_PERIOD_MS, pdTRUE,
                   NULL, repeatActionTimerCallback);
  Peak_Detector_Q =
      xQueueCreate(PEAK_DETECTOR_Q_SIZE, sizeof(PEAK_DETECTOR_Q_TYPE));
}
