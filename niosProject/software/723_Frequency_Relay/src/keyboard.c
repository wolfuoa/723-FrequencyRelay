    ...
    if (xQueueReceive(Keyboard_Q, &scanCode, portMAX_DELAY) == pdTRUE) {
      if (keyboardStatus == KEYBOARD_STATE_WAITING2) {
        if (scanCode == key_U) {
          ...
          // Set upper threshold
          if (xSemaphoreTake(Peak_Detector_thresholdMutex_X,
        ...
        if (scanCode == key_F) {
          printf("Set Upper Threshold - (U), Set Lower Threshold - (L), Cancel "
                 "Request - (Esc)\n");
        ...
        if (isEntryOk(buf, len, locationOfDecimal)) {
          ...
          printf("Set Frequency Threshold - (F), Set ABS ROC Threshold - (R), "
                 "Cancel Request - (Esc)\n");
