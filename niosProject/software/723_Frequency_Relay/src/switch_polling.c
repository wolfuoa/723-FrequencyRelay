...
#define SWITCH_POLLING_Q_TYPE uint8_t
...
int Switch_Polling_init() {
  ...
  if (xTaskCreate(Switch_Polling_handlerTask, "Switch_Polling_handlerTask",
                  configMINIMAL_STACK_SIZE, NULL, SWITCH_POLLING_TASK_PRIORITY,
                  NULL) != pdPASS) {
    return 1;
  }
  return 0;
...
static void Switch_Polling_handlerTask(void *pvParameters) {
  ...
  while (1) {
    ...
    xQueueSendToBack(Switch_Polling_Q, entry, pdFALSE);
...
static void Switch_Polling_initDataStructs() {
  ...
  Switch_Polling_Q =
      xQueueCreate(SWITCH_POLLING_Q_SIZE, sizeof(SWITCH_POLLING_Q_TYPE));
}
